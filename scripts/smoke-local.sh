...
for attempt in $(seq 1 60); do
  if listener_response="$(docker compose --project-name "${PROJECT_NAME}" --file "${COMPOSE_FILE}" exec -T listener \
    curl -fsS http://localhost:4100/ready 2>/dev/null)" && \
    printf "%s" "${listener_response}" | grep -q '"status":"ready"' && \
    printf "%s" "${listener_response}" | grep -q '"nwc_payments_v1"'; then
    listener_ready=true
    break
  fi
  sleep 2
...
if [[ "${listener_ready}" != "true" ]]; then
  echo "Listener readiness check did not pass in time." >&2
  docker compose --project-name "${PROJECT_NAME}" --file "${COMPOSE_FILE}" logs --tail 200 postgres lawallet-nwc listener >&2
  exit 1
fi
...
echo "Listener readiness check passed: ${listener_response}"
